...
_clilog_tui_cleanup() {
    dialog --msgbox "CTRL C pressed, exiting..." 5 30 2>&1 >/dev/tty
    exit 0
}
...
_clilog_tui_edit_note() {
    ...
    local editor=$(dialog --clear --menu "Which editor would you like to use?" 15 60 9 \
        1 "Vim" \
        2 "Nano" \
        3 "Neovim" \
        4 "Emacs" \
        5 "Vscode" 2>&1 >/dev/tty)
...
_clilog_tui_export_notes() {
    ...
    [[ -z "$file" ]] && {
        dialog --msgbox "Error: You did not specify the file!" 8 50 2>&1 >/dev/tty
        return
    }
